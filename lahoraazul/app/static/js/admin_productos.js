...
const inputImagenesAEliminar = document.getElementById("imagenes-a-eliminar");
    ...
    imagenes.forEach(imgNombre => {
        ...
        const btnEliminar = crearBotonEliminar(() => {
            if (confirm("¿Eliminar esta imagen adicional?")) {
                wrapper.remove();
                const nombresAEliminar = inputEliminar.value ? inputEliminar.value.split(",") : [];
...
document.querySelector(".form-producto form")
